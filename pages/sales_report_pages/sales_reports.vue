...
const filteredTransactions = computed(() => {
  // We already filtered by is_final and status in fetchTransactions,
  // so here we only need to filter by month/year and search query.
  return transactions.value
    .map((transaction) => {
      // Parse tanggal dari API - format diasumsikan YYYY-MM-DD
      const [year, month, day] = transaction.transaction_date.split("-");
...
watch([selectedMonth, selectedYear], () => {
  // Re-fetch or simply re-apply filters if data is already comprehensive
  // Since we're filtering in computed properties, just reset currentPage
  currentPage.value = 1;
});
</script>
