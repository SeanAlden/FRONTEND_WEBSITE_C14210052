  ...
  <div class="flex items-center justify-center min-h-screen bg-gray-100"> 
    <div class="w-full max-w-md p-8 bg-white rounded-lg shadow-md"> 
      ...
      <form @submit.prevent="login"> 
        <div v-if="errorMessage" class="p-3 mb-4 text-sm text-red-700 bg-red-100 border border-red-200 rounded-lg"> 
          {{ errorMessage }} 
        </div> 

        <div v-if="successMessage" class="p-3 mb-4 text-sm text-green-700 bg-green-100 border border-green-200 rounded-lg"> 
          {{ successMessage }} 
        </div> 
...
async function login() { 
  ...
  try { 
    ...
    if (error.response && error.response.status === 422) {
      ...
      errorMessage.value = "Verifikasi reCAPTCHA gagal atau tidak valid. Silakan coba lagi.";
    } else {
